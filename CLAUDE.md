...
- **Save schema** is versioned (`SAVE_SCHEMA_VERSION`, currently 4) with a `migrateSave()` chain — add a new `if (data.schemaVersion < N)` branch per future change rather than replacing the function. The `localStorage` key itself (`vectorHoleSave_v1`) never changes.
